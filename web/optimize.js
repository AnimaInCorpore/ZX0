        ...
        for (let offset = 1; offset <= max_offset; offset++) {
            if (index !== skip && index >= offset && input_data[index] === input_data[index - offset]) {
                ...
                if (last_literal[offset]) {
                    ...
                    let new_match = allocate(bits, index, offset, last_literal[offset]);
                    if (!last_match[offset] || last_match[offset].bits > new_match.bits) {
                        assign(last_match, offset, new_match);
                    }
                    if (!optimal[index] || optimal[index].bits > bits)
                        assign(optimal, index, new_match);
                }
                ...
                if (++match_length[offset] > 1) {
                    ...
                    if (!last_match[offset] || last_match[offset].index !== index || last_match[offset].bits > bits) {
                        let new_match = allocate(bits, index, offset, optimal[index - length]);
                        assign(last_match, offset, new_match);
                        if (!optimal[index] || optimal[index].bits > bits)
                            assign(optimal, index, new_match);
                    }
                ...
                if (last_match[offset]) {
                    ...
                    let new_literal = allocate(bits, index, 0, last_match[offset]);
                    assign(last_literal, offset, new_literal);
                    if (!optimal[index] || optimal[index].bits > bits)
                        assign(optimal, index, new_literal);
                }
            }
        }
